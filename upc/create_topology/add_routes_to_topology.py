...
Uses the g2 RoutingEngine directly, so the generated routes are guaranteed
to be identical to what g2 computes at runtime with routing_mode =
'foldedclos_uniform'.
...
# ---------------------------------------------------------------------------
# Add the g2 backend directory to sys.path so we can import RoutingEngine,
# LRUCache, and RunTracker directly.
# ---------------------------------------------------------------------------
_SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
_G2_DIR = os.path.normpath(os.path.join(
    _SCRIPT_DIR, '..', '..', 'extern', 'network_backend', 'g2'))

if _G2_DIR not in sys.path:
...
def generate_routes(graph, num_npus, topology_file):
    ...
    """
    # The RoutingEngine needs LRUCache and RunTracker instances.
    # We won't rely on caching — sizes are generous to avoid eviction.
    ecmp_cache = LRUCache(max_size=1)       # unused in uniform mode
    uniform_cache = LRUCache(max_size=num_npus * num_npus)
    tracker = RunTracker(network_id='route_gen',
                         possible_network_stats=[
                             'max_ecmp_paths_size',
                             'max_uniform_path_cache_size'])

    engine = RoutingEngine(
        graph=graph,
        paths={},               # no pre-computed file paths
        ecmp_paths=ecmp_cache,
        uniform_path_cache=uniform_cache,
