...
def generate_approx_topology_files(topology, config,
    ...
    n_approx_sw = (len(metadata['upper_switches'])
                   + len(metadata['tor_switches'])
                   + len(metadata.get('core_switches', set())))
    print(f"  Switches: {n_orig_sw} original  →  {n_approx_sw} approximated")
    print(f"    Merged agg nodes: {metadata['upper_switches']}")
    if metadata.get('core_switches'):
        print(f"    Core switches kept: {len(metadata['core_switches'])} (unchanged)")
...
if __name__ == '__main__':
    ...
    generate_approx_topology_files('FoldedClos', {
        'K': 4, 'bandwidth_config': fc_bw, 'bw_unit': 'GB/s',
