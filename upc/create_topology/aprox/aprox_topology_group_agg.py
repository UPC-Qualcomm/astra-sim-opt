...
def approximate_foldedclos(topo_obj):
    ...
    - Edge / ToR switches (t*)
    - All (K/2)^2 core switches (c*)  ← preserved for accuracy
    ...
    g{pod}  → c_k     :  sum over each agg switch a in the pod of
                         BW(a → c_k)   [one direction only per (a, c_k) pair]
                         (one such merged link per core switch c_k)
    ...
    # ── Scan original links to accumulate per-(edge,agg) and per-(agg,core) BW ──
    ...
    # physical link is counted exactly once.
    edge_agg_bw  = defaultdict(lambda: defaultdict(float))
    edge_agg_lat = defaultdict(lambda: defaultdict(float))
    # agg_core_bw[agg_sw][core_sw] = BW on that specific agg→core link
    agg_core_bw  = defaultdict(lambda: defaultdict(float))
    agg_core_lat = defaultdict(lambda: defaultdict(float))
    ...
    for lid, (src, dst) in topo_obj.links.items():
        ...
        # agg → core  (forward only; track per-core-switch for accurate merge)
        if src in agg_set and dst in core_set:
            agg_core_bw[src][dst]  += bw
            agg_core_lat[src][dst]  = lat
    ...
    # Copy all links that don't touch agg nodes.
    # Core switches are kept as-is; core↔agg links are replaced by g{pod}↔core links.
    for lid, (src, dst) in topo_obj.links.items():
        if src in agg_set or dst in agg_set:
            continue
    ...
    # Add merged agg layer (one g{pod} per pod, connected to actual core switches)
    upper_switches  = []
    tor_to_group    = {}
    group_to_core   = {}   # g{pod} → frozenset of core switch names it connects to
    ...
    for pod_idx, agg_list in enumerate(pods):
        ...
        # merged_agg ↔ each core switch individually
        # BW(g{pod} → c_k) = sum over all agg in pod of BW(agg → c_k)
        pod_cores = set()
        for core_sw in sorted(core_set, key=_get_num):
            total_bw = sum(agg_core_bw[a].get(core_sw, 0.0) for a in agg_list)
            if total_bw > 0:
                rep_lat = next((agg_core_lat[a][core_sw] for a in agg_list
                                if core_sw in agg_core_lat[a]), 0.0)
                ctr = _add_bidir(new_links, new_bw, new_lat, ctr,
                                 merged_agg, core_sw, total_bw, rep_lat)
                pod_cores.add(core_sw)
        group_to_core[merged_agg] = frozenset(pod_cores)
    ...
    metadata = {
        ...
        'core_switches' : frozenset(core_set),
    }
