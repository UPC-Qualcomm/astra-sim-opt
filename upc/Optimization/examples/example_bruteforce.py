...
MODEL_NAME           = "llama_8b_bruteforce"
NUM_NPUS             = 64
...
def _eval_single(args_tuple):
    ...
    (config, eval_idx, model_num, model_name, network_name,
     folder_prefix, net_sim_config, objective_type, clusters) = args_tuple
...
def main():
    ...
    search_space_path = args.search_space or os.path.join(
        ...
        "search_space", "parallelism_strategy_params_g2_intra_bruteforce.json"
    )
    ...
    # 3. Net-sim config (passed to each worker — workers create their own SimulationRunner)
    net_sim_config = {
        "sim_type": "g2",
        "topology": "FoldedClos",
        "paths_mode": "None",
    ...
    # 4. Objective (printed here; each worker creates its own instance)
    ...
    base_dir = os.environ["ASTRA_SIM_ROOT"] + "/upc"
