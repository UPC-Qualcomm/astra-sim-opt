...
def generate_naive_all_to_all(npus_count: int, comm_size: int, num_concurrent_sends: int) -> None:
    ...
    """
    dir_path = os.path.join(os.path.dirname(__file__), "..", "workload", "sends_recv")
    os.makedirs(dir_path, exist_ok=True)

    message_pairs = [(0, 8), (4, 10)]
    involved_npus = set(npu for pair in message_pairs for npu in pair)
