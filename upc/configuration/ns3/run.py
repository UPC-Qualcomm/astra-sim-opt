...
ECMP_SEED {ecmp_seed}
USE_PRECOMPUTED_ROUTES {precomputed_routes}
...
if __name__ == "__main__":
    ...
    # Precomputed routes options
    precomputed_routes = [0, 1]
    ...
    # DCQCN variations
    dcqcn_variants = ['dcqcn', 'dcqcn_vwin'] #['dcqcn', 'dcqcn_paper', 'dcqcn_vwin', 'dcqcn_paper_vwin']
    ...
    for (cc, u_tgt_float, mi) in params:
        for bfsz in buffer_sizes:
            for pc in precomputed_routes:
                # ECMP seeds to test when precomputed_routes is 0
                ecmp_seeds = [25]
                if pc == 0:
                    ecmp_seeds.extend([42, 1337])

                for seed in ecmp_seeds:
                    ...
                    config_name = f"{project_root}upc/configuration/ns3/FoldedClos_16_config{config_idx}.txt"
                    ...
                    if cc == "pfc":
                        ...
                        cc_name = "%s%dmi%d_buf%d" % (cc, utgt_int, mi, bfsz)
                    elif cc == "dctcp":
                        mode = 8
                        ai = 10
                        hai = ai
                    ...
                    config = config_template.format(
                        ...
                        enable_tr=enable_tr, min_rate=min_rate, enable_qcn=enable_qcn, ecmp_seed=seed
                    )
