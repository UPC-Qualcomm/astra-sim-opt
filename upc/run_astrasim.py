...
def run_astrasim(workload_path, system, network, memory, output_dir):
    ...
    cmd = f"{astrasim_bin} --system-configuration={system} --workload-configuration={workload_path} --network-configuration={network} --remote-memory-configuration={memory} --comm-group-configuration={workload_path}.json > {log}"
    print(cmd)
...
if __name__ == '__main__':
    ...
    func = partial(run_astrasim, system = args.system, network = args.network, memory = args.memory, output_dir = args.output_dir)
