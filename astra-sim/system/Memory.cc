...
bool Memory::check_free_memory(
    ...
    if (node->type() == ChakraNodeType::COMP_NODE) {
        ...
        required_mem = node->get_attr_msg("y_tensor_size").int64_val() *
                       this->get_num_bytes();
    } else {
        /*for (auto attr : node->getChakraNode()->attr()) {
        ...
        required_mem = node->comm_size() * this->get_num_bytes();
    }
...
void Memory::update_consumed_memory(
    std::shared_ptr<Chakra::FeederV3::ETFeederNode> node, const std::unordered_set<Chakra::FeederV3::NodeId> child_nodes, int sys_id) {

    this->is_oom = this->is_oom ? this->is_oom : !this->check_free_memory(node);

    std::vector<uint64_t> children_ids(child_nodes.begin(), child_nodes.end());
    ...
    if (this->is_forward_pass) {
        if (is_tensor) {
            if (node->type() == ChakraNodeType::COMP_NODE && !is_embedding) {
                ...
                uint64_t tensor_size =
                    node->get_attr_msg("y_tensor_size").int64_val() *
                    other_mem_bytes;
                this->max_parameter_memory += tensor_size;
            }
        ...
        if (!is_tensor && !is_embedding &&
            ...
            uint64_t tensor_size =
                ...
                other_mem_bytes;
            // this->gradient_memory.removeChildIdFromAll(node_id);
            this->gradient_memory.addNode(node_id, tensor_size, children_ids);
            this->acumilated_gradient_memory += tensor_size;
        }
